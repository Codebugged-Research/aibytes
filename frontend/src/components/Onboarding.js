...
import { setOnboarded, setUser, setRolePref } from '../utils/storage';
...
export const Onboarding = ({ onComplete }) => {
  ...
  const [step, setStep] = useState('welcome'); // welcome | loading | phone | email | otp | details | role | success
  ...
  const [pendingFinish, setPendingFinish] = useState(null); // { method, payload } queued until the role step completes
  ...
  // Account creation is done, but we still want the learner's role
  // before celebrating — queue the finish() call for the role step.
  const goToRole = (method, payload) => {
  ...
  const handleOAuth = (which) => {
    ...
    setTimeout(() => goToRole(which, mock), 1500);
  };
